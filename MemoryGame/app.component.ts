...
import { CardProps } from './flip-card/flip-card.component';
...
export class AppComponent implements OnInit {
  ...
  setupGame() {
    for (let index = 0; index < 8; index++) {
      let f = true;
      while (f) {
        let i = Math.ceil(Math.random()*this.names.length);
    ...
    this.itemIndex = this.itemIndex.concat(this.itemIndex);
    this.itemIndex = this.shuffleIndex(this.itemIndex);
    
    this.itemIndex.forEach(el => {
      this.itemArray.push(this.names[el]);
    });
  }

  shuffleIndex(arr: number[]) {
    let shuffledIndex: number[] = [];
    for (let index = 0; index < arr.length; index++) {
      let f = true;
      while (f) {
        let i = Math.floor(Math.random()*arr.length);
        if (!shuffledIndex.includes(i)) {
  ...
  onCardClicked(event: CardProps) {
    if (this.flippedItems.length < 2) {
      event.flipped = true;
      event.disabled = true;
      this.flippedItems.push(event);
    }
    if (this.flippedItems.length === 2) {
      this.turns += 1;

      setTimeout(() => {
        if (this.flippedItems[0].index === this.flippedItems[1].index) {
          this.flippedItems[0].closed = true;
          this.flippedItems[1].closed = true;
          this.score += 1;
          if (this.score === 8) this.playAgain = true;
          this.flippedItems = [];
        } else {
          this.flippedItems[0].flipped = false;
          this.flippedItems[0].disabled = false;
          this.flippedItems[1].flipped = false;
          this.flippedItems[1].disabled = false;
